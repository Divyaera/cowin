...
def main():
    ...
    try:
        ...
        if args.token:
            ...
            otp_pref = input("\nDo you want to enter OTP automatically (y/n Default y) (Needs OTP reader enabled in your phone): ")
            otp_pref = otp_pref if otp_pref else "y"
            while token is None:
                if otp_pref=="y":
                    try:
                        if not bucket_str:
                            bucket_str = input("\nEnter kvdb.io bucket string number (Eg.SdgQXXWXNWhDzMqqT4D5Mx): ")
                        token = generate_token_OTP(mobile, base_request_header, bucket_str)
